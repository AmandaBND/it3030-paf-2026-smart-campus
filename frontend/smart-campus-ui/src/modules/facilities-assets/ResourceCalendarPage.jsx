...
export default function ResourceCalendarPage() {
  ...
  const busyByResource = useMemo(() => {
    ...
    for (const b of busySlots || []) {
      ...
      if (Number.isNaN(rid) || bs == null || be == null) continue
  ...
  const slotColor = (idx) => {
    const slot = slots[idx]
    if (!slot) return 'rgba(212, 175, 55, 0.05)'
    const isBusy = resourceBusy.some((b) => overlaps(slot.startMin, slot.endMin, b.startMin, b.endMin))
    if (isBusy) return 'rgba(220, 80, 80, 0.85)'

    if (selectedRange && idx >= selectedRange.start && idx <= selectedRange.end) {
      return 'rgba(36, 96, 200, 0.6)'
    }
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: isSelected ? '2px solid var(--color-secondary)' : '1px solid rgba(212, 175, 55, 0.2)',
                  borderRadius: 6,
...
function memoizeResourceBusy(resourceId, busyByResource) {
  if (!resourceId) return []
  return busyByResource.get(Number(resourceId)) || []
}
