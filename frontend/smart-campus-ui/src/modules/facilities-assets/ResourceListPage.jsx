...
export default function ResourceListPage() {
  ...
  const handleSlotClick = (r, index) => {
    ...
    const busyIntervals = busyByResource.get(r.id) || []
  ...
  return (
    <div className="page">
          ...
          return (
            <div key={r.id} className="card" style={{ padding: '1rem 1.05rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
                <span className="badge badge-ok" style={{ background: 'linear-gradient(135deg, rgba(36, 96, 200, 0.12), rgba(36, 96, 200, 0.06))' }}>
                  {r.type}
                </span>
              ...
              <div style={{ marginTop: '0.85rem', display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setPreviewImageUrl(getImageUrl(r.imageUrl))
                    setPreviewResourceName(r.name)
                  }}
                  disabled={!r.imageUrl}
                  title={r.imageUrl ? 'View resource image' : 'No image available'}
      ...
      {previewImageUrl && (
        ...
        >
          <div className="card" style={{ maxWidth: 760, width: '100%', padding: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', alignItems: 'center' }}>
              <h3 style={{ margin: 0 }}>{previewResourceName} - Image</h3>
              <button
                type="button"
                className="btn btn-ghost"
                onClick={() => {
            ...
            <img
              src={previewImageUrl}
              alt={`${previewResourceName} preview`}
              style={{
                width: '100%',
                maxHeight: '70vh',
                objectFit: 'contain',
